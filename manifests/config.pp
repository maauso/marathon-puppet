...
class marathon::config(
  ...
  $conf_dir_name                    = 'conf',
  $conf_dir_base_artifact           = '/var/lib/marathon',
  $conf_dir_artifact                = '/var/lib/marathon/artifact',
  $owner                            = 'root',
  $group                            = 'root',
  $master                           = undef,
  $zookeeper                        = undef,
  ...
  $artifact_store                   = $marathon::artifact_store,
  ...
  if $artifact_store {
    file { "${conf_dir_base_artifact}":
      ensure => directory,
      owner  => $owner,
      group  => $group,
    }
    file { "${conf_dir_artifact}":
      ensure  => directory,
      owner   => $owner,
      group   => $group,
      require => File[ "${conf_dir_base_artifact}" ],
    }
    file {"${conf_dir_base}/${conf_dir_name}/artifact_store":
      ensure  => present,
      content => $artifact_store,
      owner   => $owner,
      group   => $group,
      require => File[ "${conf_dir_artifact}" ],
    }
  }

  if $event_subscriber {  
    file {"${conf_dir_base}/${conf_dir_name}/event_subscriber":
      ensure  => present,
      content => $event_subscriber,
      owner   => $owner,
      group   => $group,
    } 
  }
  ...
  if $mesos_role {
    file {"${conf_dir_base}/${conf_dir_name}/mesos_role":
      ensure  => present,
      content => $mesos_role,
      owner   => $owner,
      group   => $group,
